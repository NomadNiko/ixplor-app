...
export default function DayColumn({ date, items, onItemClick, isVendorView }: DayColumnProps) {
  ...
  return (
    <Box sx={{ 
      ...
      <Box sx={{ 
        p: 1,
        textAlign: 'center',
        bgcolor: theme => isCurrentDay ? alpha(theme.palette.primary.main, 0.1) : 'transparent',
        borderBottom: 1,
        borderColor: 'divider'
      }}>
          ...
          >
            ...
            {isVendorView && (
              <Typography 
                ...
                sx={{ 
                  display: 'block',
                  color: item.quantityAvailable > 0 ? 'success.main' : 'error.main'
                }}
              >
